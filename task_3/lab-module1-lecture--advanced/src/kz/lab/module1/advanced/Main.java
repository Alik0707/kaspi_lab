...
public class Main {
    ...
    public static void main(String[] args) {
        //taskImmutable();
        //taskRecord();
        taskLambda(); // демонстрация работы отправки email
    }
...
//        System.out.println(greatStudent.getName());
    ...
    private static void taskRecord() {
        ...
        try {
            // создаём Order с корректными данными
            Order order = new Order(100, 50, LocalDateTime.now().minusDays(1), items);
            System.out.println(order);

            // попытка изменить items вызовет ошибку (immutable)
            // order.items().add("d"); // java.lang.UnsupportedOperationException
    ...
    public static void sendEmails(String title, String content, List<Customer> customers, EmailSender emailSender) {
        customers.stream()
                .map(c -> new Email(c.email(), title + c.name(), content)) // формируем email с именем
                .forEach(emailSender::send); // отправляем через lambda
    }
}
